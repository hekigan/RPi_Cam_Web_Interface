...
TMPDIR=${MACRODIR}/../tmp/thumbnails-${RANDOM}/
mkdir $TMPDIR

echo "macros/thumbnails-generator.sh"
echo "Create image gallery from video"
echo "ffmpeg -ss ${OFFSET} -i ${INPUT} -f image2 -vf fps=fps=${FPS} ${TMPDIR}thumb%04d.jpg"
#ffmpeg -ss $OFFSET -i $INPUT -f image2 -vf "drawtext=fontsize=45:fontcolor=white::shadowcolor=black:shadowx=2:shadowy=2:box=1:boxcolor=black@0:x=(W-tw)/40:y=H-th-10:text='%{pts\:gmtime\:0\:%H\\\\\\:%M\\\\\:%S}'" fps=fps=$FPS ${TMPDIR}thumb%04d.jpg
